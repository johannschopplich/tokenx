  ...
  describe('accented and non-Latin alphabets', () => {
    it('prices German text', () => {
      expect(measureBucket(BUCKETS.german)).toMatchInlineSnapshot(`
        ...
        medium  95 → 92    -3.2%  Ursprünglich konnten sich nur gut situierte B…
        mean               -5.3%
        mean |dev|         12.4%"
      `)
    })
    ...
    it('prices French and Spanish text', () => {
      expect(measureBucket(BUCKETS.romance)).toMatchInlineSnapshot(`
        "short   24 → 22    -8.3%  Les caféiers sont des arbustes des régions tr…
        short   14 → 11   -21.4%  La consommation de café s'étendit à l'Égypte.
        short   13 → 14    +7.7%  Les pamphlets et libelles sont distribués dan…
        short   12 → 12     0.0%  Dependiendo de la receta hay diversas formas …
        short   18 → 15   -16.7%  Tienen hojas persistentes y opuestas y bajo u…
        short   14 → 14     0.0%  Produce un café fino y aromático, y necesita …
        medium  69 → 62   -10.1%  Le café est une boisson énergisante psychotro…
        medium  93 → 92    -1.1%  El café es una bebida que se obtiene mediante…
        mean               -6.2%
        mean |dev|          8.2%"
      `)
    })
    ...
    it('prices Polish text', () => {
      expect(measureBucket(BUCKETS.slavicLatin)).toMatchInlineSnapshot(`
        ...
        short   16 → 10   -37.5%  Nazwa kawy pochodzi prawdopodobnie od arabski…
        ...
        medium  81 → 71   -12.3%  Kawa – napój sporządzany z palonych, a następ…
        mean              -17.7%
        mean |dev|         17.7%"
      `)
    })
    ...
    it('prices English text at the default ratio', () => {
      expect(measureBucket(BUCKETS.english)).toMatchInlineSnapshot(`
        "short   11 → 11     0.0%  Coffee is a beverage brewed from roasted grou…
        short   14 → 14     0.0%  It is usually served hot, although chilled or…
        ...
        medium  62 → 70   +12.9%  Coffee has become a vital cash crop for many …
        mean               +9.0%
        mean |dev|          9.0%"
      `)
    })
  ...
  describe('word length', () => {
    it('prices words of seven to ten characters', () => {
      expect(measureBucket(BUCKETS.wordsSevenToTen)).toMatchInlineSnapshot(`
        ...
        short  16 → 13   -18.8%  squeezed permanent leverage balloon promise w…
        mean              +3.1%
        mean |dev|        21.9%"
      `)
    })
  ...
  describe('structural segments', () => {
    ...
    it('prices line breaks against their wrapped equivalent', () => {
      expect(measureBucket(BUCKETS.lineBreaks)).toMatchInlineSnapshot(`
        ...
        medium  27 → 27     0.0%  alice: are we still on for the review?\\nbob: …
        medium  30 → 33   +10.0%  The estimate walks the text once, splitting i…
    ...
    it('prices digit runs', () => {
      expect(measureBucket(BUCKETS.numeric)).toMatchInlineSnapshot(`
        ...
        short  16 → 15    -6.3%  Order #48291 shipped, tracking 94055118992231…
        mean             -11.5%
        mean |dev|        11.5%"
      `)
    })
    ...
    it('prices code and URLs', () => {
      expect(measureBucket(BUCKETS.code)).toMatchInlineSnapshot(`
        ...
        short  11 → 15   +36.4%  https://example.com/path/to/resource?query=1
